        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle" id="employes-table">
                ...
                <tr class="clickable-row" onclick="window.location.href='profil_employe.php?id=<?= $e['id'] ?>'" data-employee-id="<?= $e['id'] ?>">
                    ...
                    <!-- STATUT (évalué selon le badge actif) -->
                    <?php
                        // Vérifier s'il existe un badge actif pour cet employé
                        $stmtBadge = $pdo->prepare("SELECT COUNT(*) FROM badge_tokens WHERE employe_id = ? AND status = 'active' AND expires_at > NOW()");
                        $stmtBadge->execute([$e['id']]);
                        $badgeCount = (int)$stmtBadge->fetchColumn();
                        $isBadgeActive = $badgeCount > 0;

                        // Le statut affiché suit l'état du badge (conformément à la demande)
                        $displayActive = $isBadgeActive;
                    ?>
                    <td>
                        <span class="badge status-badge bg-<?= $displayActive ? 'success' : 'danger' ?>"
                              title="Statut DB: <?= htmlspecialchars($e['statut'] ?? 'N/A') ?>">
                            <?= $displayActive ? 'Actif' : 'Inactif' ?>
                        </span>
                        <?php if (!$isBadgeActive && ($e['statut'] ?? '') === 'actif'): ?>
