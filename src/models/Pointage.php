...
if (!class_exists(__NAMESPACE__ . '\\Pointage', false)) {
    class Pointage {
        ...
        /**
         * Crée un pointage (méthode principale)
         */
        ...
        /**
         * Enregistre un pointage avec validation et calculs automatiques
         */
        public function record(array $data): int {
            ...
            try {
                ...
                $sql = "INSERT INTO pointages (
                    employe_id, type, date_heure, date_pointage, location_lat, location_lng,
                    device_info, ip_address, badge_token_id, status, statut,
                    worked_hours, break_duration, overtime_hours, is_late,
                    late_minutes, notes, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                
                $dateHeure = $data['timestamp'] ?? ($data['date_pointage'] ?? date('Y-m-d H:i:s'));
                ...
                $stmt->execute([
                    ...
                    $dateHeure,
                    $dateHeure,
                    $data['latitude'] ?? $data['location_lat'] ?? null,
                    $data['longitude'] ?? $data['location_lng'] ?? null,
        ...
        private function calculatePointageData(array $data): array {
            ...
            $type = $data['type'];
            $timestamp = new DateTime($data['timestamp'] ?? $data['date_pointage'] ?? 'now');
            ...
            if ($type === self::TYPE_DEPARTURE) {
                ...
                if ($arrival) {
                    ...
                    $breakDuration = $this->calculateBreakDurationBetween($employeeId, 
        ...
        public function getLastByEmploye(int $employeId): ?array {
            $sql = "SELECT *, COALESCE(date_heure, date_pointage, created_at) AS timestamp 
                    FROM pointages 
                    WHERE employe_id = ? 
        ...
        /**
         * Récupère le dernier pointage d'un type spécifique
         */
        ...
        /**
         * Calcule la durée totale des pauses pour une journée
         */
        public function calculateBreakDuration(int $employeeId, string $date): int {
            return $this->calculateBreakDurationForDate($employeeId, $date);
        }

        /**
         * Calcule la durée totale des pauses pour une date spécifique
         */
        private function calculateBreakDurationForDate(int $employeeId, string $date): int {
            $sql = "SELECT 
                SUM(CASE 
         ...
         */
        public function getByEmployeAndPeriod(int $employeeId, string $startDate, string $endDate, int $limit = 100): array {
            ...
            foreach ($rows as $row) {
                ...
                if ($isArrival) {
                    ...
                    if ($lastArrival !== null) {
                        ...
                        $breaks = $this->calculateBreakDurationBetween($employeeId, 
                            date('Y-m-d H:i:s', $lastArrival), 
                            date('Y-m-d H:i:s', $ts));
        ...
        public function getEmployeeMonthlySummary(int $employeeId, ?int $month = null, ?int $year = null): array {
            ...
            $prevEndDate = sprintf('%04d-%02d-%02d', $prevYear, $prevMonth, 
                (int)date('t', strtotime($prevStartDate)));
            $prevTotalSeconds = $this->getTotalWorkSecondsForPeriod($employeeId, $prevStartDate, $prevEndDate);
