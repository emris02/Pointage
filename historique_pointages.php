<?php

require_once 'src/config/bootstrap.php';
require_once 'src/services/AuthService.php';

// Déterminer le rôle de l'utilisateur
$is_admin = isset($_SESSION['role']) && in_array($_SESSION['role'], ['admin', 'super_admin']);
$employe_id = $_SESSION['employe_id'] ?? null;

if ($is_admin) {
    // Admin : voir tout l'historique
    $sql = "SELECT 
                e.nom,
                e.prenom,
                e.email,
                e.departement,
                e.poste,
                p.date_heure,
                p.type,
                p.retard_minutes,
                p.est_justifie,
                p.commentaire,
                p.temps_total
            FROM pointages p
            INNER JOIN employes e ON p.employe_id = e.id
            ORDER BY p.date_heure DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
} elseif ($employe_id) {
    // Employé : voir uniquement son historique
    $sql = "SELECT 
                e.nom,
                e.prenom,
                e.email,
                e.departement,
                e.poste,
                p.date_heure,
                p.type,
                -- latest retard info from retards table
                (SELECT r.raison FROM retards r WHERE r.pointage_id = p.id ORDER BY r.date_soumission DESC LIMIT 1) as retard_raison,
                (SELECT r.details FROM retards r WHERE r.pointage_id = p.id ORDER BY r.date_soumission DESC LIMIT 1) as retard_details,
                (SELECT r.statut FROM retards r WHERE r.pointage_id = p.id ORDER BY r.date_soumission DESC LIMIT 1) as retard_statut,
                p.est_justifie,
                p.commentaire,
                p.temps_total
            FROM pointages p
            INNER JOIN employes e ON p.employe_id = e.id
            WHERE p.employe_id = :employe_id
            ORDER BY p.date_heure DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['employe_id' => $employe_id]);
} else {
    // Non connecté : pas d'accès
    header('Location: login.php');
    exit;
}

$pointages = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Historique des Pointages</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body {
            background-color: #f9f9f9;
            padding: 0.5rem;
        }
        h2 {
            text-align: center;
            margin-bottom: 2rem;
        }
        table {
            font-size: 0.95rem;
        }
        th {
            background-color: #007bff;
            color: white;
            text-align: center;
        }
        td {
            text-align: center;
            vertical-align: middle;
        }
        .badge {
            font-size: 0.8rem;
        }
    </style>
</head>
<body>

<h2>Historique complet des pointages des employés</h2>

<div class="table-responsive">
    <table class="table table-bordered table-striped shadow">
        <thead>
            <tr>
                <th>Nom</th>
                <th>Prénom</th>
                <th>Email</th>
                <th>Département</th>
                <th>Poste</th>
                <th>Date & Heure</th>
                <th>Type</th>
                <th>Temps Travaillé</th>
                <th>Retard (min)</th>
                <th>Cause / Commentaire</th>
                <th>Justifié ?</th>
                <th>Commentaire</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($pointages as $p): ?>
                <tr>
                    <td><?= htmlspecialchars($p['nom']) ?></td>
                    <td><?= htmlspecialchars($p['prenom']) ?></td>
                    <td><?= htmlspecialchars($p['email']) ?></td>
                    <td><?= htmlspecialchars($p['departement'] ?? '-') ?></td>
                    <td><?= htmlspecialchars($p['poste'] ?? '-') ?></td>
                    <td><?= htmlspecialchars($p['date_heure']) ?></td>
                    <td>
                        <span class="badge <?= $p['type'] == 'arrivee' ? 'bg-success' : 'bg-warning text-dark' ?>">
                            <?= ucfirst($p['type']) ?>
                        </span>
                    </td>
                    <td><?= $p['temps_total'] ?? '—' ?></td>
                    <td><?= (!empty($p['retard_minutes']) && $p['retard_minutes'] > 0) ? htmlspecialchars((string)$p['retard_minutes']) : '—' ?></td>
                    <td><?= htmlspecialchars($p['retard_raison'] ?? $p['retard_details'] ?? $p['commentaire'] ?? '-') ?></td>
                    <td>
                        <?php if (!empty($p['retard_statut']) && $p['retard_statut'] === 'approuve') : ?>
                            <span class="badge bg-success">Oui</span>
                        <?php elseif (!empty($p['retard_statut']) && $p['retard_statut'] === 'en_attente') : ?>
                            <span class="badge bg-warning text-dark">En attente</span>
                        <?php else: ?>
                            <?= !empty($p['est_justifie']) ? '<span class="badge bg-success">Oui</span>' : '<span class="badge bg-danger">Non</span>' ?>
                        <?php endif; ?>
                    </td>
                    <td><?= htmlspecialchars($p['commentaire'] ?? '-') ?></td>
                </tr>
            <?php endforeach; ?>
            <?php if (empty($pointages)): ?>
                <tr><td colspan="14">Aucun pointage trouvé.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>

</body>
</html>

